function showBrandType(){
    ...
    header += '<thead><tr ><th colspan="4" class="bg-primary" style="color:white">ยี่ห้ออุปกรณ์</th></tr><tr class="bg-primary" style="color:white">';
    header += '<th>brand_id</th><th>brand_name</th><th></th></tr></thead><tbody>'
    ...
    $.getJSON("http://localhost/stationarySystem/api/brand/read.php", function(data){
        ...
        $.each(data.records, function(key, val) {
            read_select_type += '<tr>';
                ...
                button += '<div class="dropdown-menu" aria-labelledby="dropdownMenuLink">';
                    button += '<a class="dropdown-item" data-toggle="modal" value="'+val.id+'" data-target="#addBrand" >เพิ่ม</a>';
                    button += '<a class="dropdown-item" data-toggle="modal"   data-target="#editModal" onclick="editBrand('+"'"+val.id+"'"+')" >แก้ไข</a>';
                    button += '<a class="dropdown-item" data-toggle="modal" data-target="#deleteModal"  onclick="deleteBrand('+"'"+val.id+"'"+')"  >ลบ</a>';
                button += '</div>';
            button += '</div></td>';
        ...
        $(".brand_type").append(read_select_type);
    });
