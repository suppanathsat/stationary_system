function showUnitType(){
    ...
    header += '<thead><tr ><th colspan="4" class="bg-primary" style="color:white">หน่วยอุปกรณ์</th></tr><tr class="bg-primary" style="color:white">';
    header += '<th>unit_id</th><th>unit_name</th><th></th></tr></thead><tbody>'
    ...
    $.getJSON("http://localhost/stationarySystem/api/unit/read.php", function(data){
        ...
        $.each(data.records, function(key, val) {
            read_select_type += '<tr>';
                ...
                button += '<div class="dropdown-menu" aria-labelledby="dropdownMenuLink">';
                    button += '<a class="dropdown-item" data-toggle="modal" value="'+val.id+'" data-target="#addUnit" >เพิ่ม</a>';
                    button += '<a class="dropdown-item" data-toggle="modal"   data-target="#editModal" onclick="editUnit()" >แก้ไข</a>';
                    button += '<a class="dropdown-item" data-toggle="modal" data-target="#deleteModal"  onclick="deleteUnit()"  >ลบ</a>';
                button += '</div>';
            button += '</div></td>';
        ...
        $(".unit_type").append(read_select_type);
    });
