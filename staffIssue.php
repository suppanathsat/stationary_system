...
<body id="page-top">
                  ...
                  <div class="row">
                      <div class="form-group col-md-2">
                        ...
                        <select class="form-control" id="status_type">
                          ...
                          <option id="อนุมัติแล้ว">อนุมัติ</option>
                          <option id="กำลังอนุมัติ">ไม่อนุมัติ</option>
  ...
  <script id="myScript">
    ...
    var script = "";
    var sta = $.getJSON("http://localhost/stationarySystem/api/issue_status/readByStaffID.php?staff_id="+"<?php echo $_SESSION['staff_id'];?>", function(data){
        ...
        script = "<script>";
        script += "function updateStatus(tran_id,id){";
          script += "alert('555+');";              
        script += "}";
        script += "";
        read_select_sta += script;
      ...
      function  staffSearchTran(tran_id){
        var read_select_sta = '<table class="table table-bordered " id="dataTable" width="100%" cellspacing="0" style="text-align: center;">';
        read_select_sta += '<thead><tr class="bg-primary" style="color:white">';
        read_select_sta += '<th>รหัส</th><th>อุปกรณ์</th><th>จำนวน</th><th>ในสต๊อค</th><th>สถานะ</th></tr></thead><tbody>';
        var i = 0;
        
        var sta = $.getJSON("http://localhost/stationarySystem/api/issue_status/staffSearchTran.php?tran_id="+tran_id, function(data){
      ...
      function   staffSearchByStatus(status_name){
        var read_select_sta = '<table class="table table-bordered " id="dataTable" width="100%" cellspacing="0" style="text-align: center;">';
        read_select_sta += '<thead><tr class="bg-primary" style="color:white">';
        read_select_sta += '<th>รหัส</th><th>อุปกรณ์</th><th>จำนวน</th><th>ในสต๊อค</th><th>สถานะ</th></tr></thead><tbody>';
       
        console.log("http://localhost/stationarySystem/api/issue_status/staffSearchByStatus.php?status_name="+status_name);
        var sta = $.getJSON("http://localhost/stationarySystem/api/issue_status/staffSearchByStatus.php?status_name="+status_name, function(data){
          var len = 0;
                $.each(data.records, function(key, val) {
                  len++;
                  read_select_sta += "<tr class='starow'  value='"+val.staff_id+"'>";
